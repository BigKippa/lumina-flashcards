...
export function LoginScreen({ onLogin, showToast }: LoginScreenProps) {
  ...
  useEffect(() => {
    ...
    if (params.get('invite') === 'true') {
      ...
      setIsInvite(true);
      setRole('user'); // Force student role on invite
  ...
  const handleGoogleSignup = () => {
    ...
    setTimeout(() => {
      ...
      setUsername('alex_student');
      setIsSendingCode(false);
      showToast("Google profile retrieved successfully", "success");
    }, 1200);
  };
      ...
      <div className="w-full flex-grow flex flex-col items-center justify-center bg-background p-4 relative overflow-hidden">
        ...
        <div className="max-w-md w-full p-8 rounded-2xl bg-gradient-to-br from-color5/80 to-color4/50 text-color5-foreground backdrop-blur-xl border border-color5/20 shadow-2xl relative z-10 animate-in fade-in zoom-in duration-500">
          <div className="flex flex-col items-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-inherit text-center">Setup Your Account</h2>
      ...
      <div className="w-full flex-grow flex flex-col items-center justify-center bg-background p-4 relative overflow-hidden">
        ...
        <div className="max-w-md w-full p-8 rounded-2xl bg-gradient-to-br from-color5/80 to-color4/50 text-color5-foreground backdrop-blur-xl border border-color5/20 shadow-2xl relative z-10 animate-in fade-in zoom-in duration-500">
          <div className="flex flex-col items-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-inherit text-center">{t('auth.verify_email')}</h2>
            <p className="opacity-80 mt-2 text-center text-sm">
              We sent a 6-digit code to <span className="font-semibold text-inherit">{email}</span>
            </p>
          </div>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full px-6 py-4 rounded-xl bg-background text-foreground border border-input focus:border-primary focus:ring-1 focus:ring-primary outline-none text-center text-2xl tracking-[0.5em] font-mono"
                placeholder="000000"
                maxLength={6}
                autoFocus
              />
        ...
        <div className="max-w-[480px] w-full p-8 rounded-2xl bg-gradient-to-br from-color5/80 to-color4/50 text-color5-foreground backdrop-blur-xl border border-color5/20 shadow-2xl animate-in fade-in zoom-in duration-500">
          ...
          {authMode === 'login' && (
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-5 py-3 rounded-xl bg-background/50 text-foreground border border-input focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-muted-foreground/50 text-base"
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-5 py-3 pr-12 rounded-xl bg-background/50 text-foreground border border-input focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-muted-foreground/50 text-base"
              ...
              {error && (
                <div className="bg-destructive/10 p-3 rounded-lg flex flex-col items-center gap-2">
                  ...
                  {error.includes("User not found") && (
                    <>
                      <span className="text-xs opacity-80 uppercase">{t('common.or') || "or"}</span>
                      <button
                        type="button"
                        onClick={() => {
                          setAuthMode('register');
                        ...
                        className="text-color5 text-sm font-bold hover:underline transition-all"
                      >
              ...
              <button
                type="submit"
                className="w-full py-3 px-5 rounded-xl bg-primary text-primary-foreground font-bold hover:bg-primary/90 transition-all active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-primary/20 text-sm "
              >
                {t('auth.login')} <ArrowRight className="w-5 h-5 text-primary-foreground" />
              </button>
              <button
                type="button"
          ...
          {authMode === 'register' && (
            <form onSubmit={handleStartRegister} className="space-y-6">
              <button
                ...
                disabled={isSendingCode}
                className="w-full py-3.5 px-6 rounded-xl bg-white text-zinc-900 font-bold hover:bg-gray-100 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 shadow-md"
              >
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label htmlFor="firstName" className="text-xs font-bold text-primary uppercase tracking-wider ml-1">
                      First Name
                    </label>
                    <input
                      id="firstName"
                  ...
                  <div className="space-y-2">
                    <label htmlFor="lastName" className="text-xs font-bold text-primary uppercase tracking-wider ml-1">
                      Last Name
                    </label>
                    <input
                      id="lastName"
                ...
                <div className="space-y-2">
                  <label htmlFor="confirmPassword" className="text-xs font-bold text-primary uppercase tracking-wider ml-1">
                    Confirm Password
                  </label>
                  <div className="relative">
                    <input
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        ...
                        className={`p-4 rounded-xl border-2 flex items-center justify-center gap-2 transition-all cursor-pointer ${role === 'user'
                          ? 'bg-color2 border-color2 text-white shadow-lg shadow-color2/30 scale-[1.02]'
                          : 'bg-secondary hover:bg-secondary/80 border-primary/10 text-foreground shadow-sm'
                          }`}
                      >
                        <User className="w-5 h-5" />
                        <span className="font-bold text-lg">{t('auth.role_learner')}</span>
                      </button>
                      <button
                        type="button"
                        onClick={() => setRole('tutor')}
                        className={`p-4 rounded-xl border-2 flex items-center justify-center gap-2 transition-all cursor-pointer ${role === 'tutor'
                          ? 'bg-color3 border-color3 text-white shadow-lg shadow-color3/30 scale-[1.02]'
                          : 'bg-secondary hover:bg-secondary/80 border-primary/10 text-foreground shadow-sm'
                          }`}
                      >
                        <GraduationCap className="w-5 h-5" />
                        <span className="font-bold text-lg">{t('auth.role_tutor')}</span>
                      </button>
                    </div>
                  </div>
                )}
              ...
              >
                {isSendingCode ? "Sending Code..." : t('common.next')} <ArrowRight className="w-5 h-5 text-primary-foreground" />
              </button>
              <button
                type="button"
                onClick={() => { setAuthMode('initial'); setError(null); }}
